...
def task1(city_day_df, station_day_df, stations_df, city_hour_df):
    ...
    city_day["Date"] = pd.to_datetime(city_day["Date"])
    city_day["Year"] = city_day["Date"].dt.year
    city_day["Festival"] = "None"

    fest_order = ["Diwali", "Dussehra", "Holi"]
    window = 7

    for fest in fest_order:
        fest_date_series = city_day["Year"].map(
            {y: pd.to_datetime(d) for y, d in festival_dates[fest].items()}
        )

        diff = (city_day["Date"] - fest_date_series).dt.days

        in_window = diff.between(-window, window) & fest_date_series.notna()

        labels = np.select(
            [diff.eq(0), diff.lt(0), diff.gt(0)],
            [f"{fest}_Day", f"{fest}_Before", f"{fest}_After"],
            default="None"
        )

        update_mask = city_day["Festival"].eq("None") & in_window
        city_day.loc[update_mask, "Festival"] = labels[update_mask.to_numpy()]
    ...
    diwali_summary = diwali_df.groupby("Festival")["AQI"].mean().reset_index()
    holi_summary = holi_df.groupby("Festival")["AQI"].mean().reset_index()
    dussehra_summary = dussehra_df.groupby("Festival")["AQI"].mean().reset_index()
...
def task3(station_hour_df,city_day_df, station_day_df, stations_df, city_hour_df):
    ...
    subset = station_quality.sort_values('coverage_pct', ascending=True).head(100)
    ...
    fig = px.imshow(
        ...
        aspect='auto',
        text_auto='.1f',
        title="Pollutant-wise Coverage - 100 Stations"
    )
