...
def load_data():
    ...
    city_hour_df = pd.read_csv(
    "https://huggingface.co/datasets/khush-thakkar-09/station_hour/resolve/main/city_hour.csv.gz",
    compression="gzip",
    low_memory=False)

    return stations_df, station_day_df, station_hour_df, city_day_df, city_hour_df


stations_df, station_day_df, station_hour_df, city_day_df, city_hour_df = load_data()

def task1(city_day_df, station_day_df, station_hour_df, stations_df):
...
def task2(city_day_df, station_day_df, station_hour_df, stations_df):
    import plotly.express as px
    import plotly.graph_objects as go
    import numpy as np
...
def task3(station_hour_df,city_day_df, station_day_df, stations_df):
...
def task4(city_day_df, station_day_df, station_hour_df, stations_df):
    city_day = city_day_df.dropna(subset=["AQI_Bucket"])
...
if selection == "Introduction":
    ...
    task1(city_day_df, station_day_df, station_hour_df, stations_df)

elif selection == "Task 2":
    set_page_class("page-task")
    task2(city_day_df, station_day_df, station_hour_df, stations_df)

elif selection == "Task 3":
    set_page_class("page-task")
    task3(station_hour_df, city_day_df, station_day_df, stations_df)

elif selection == "Task 4":
    set_page_class("page-task")
    task4(city_day_df, station_day_df, station_hour_df, stations_df)
